...
public class ExpertRegistrationAction extends AbstractTransactionalAction {
    ...
    private  List<UserProgram> allProgram;
...
//            getProgramService().saveExpertPrograms(userAccount, programs);
          if (!userAccount.roleIs(ApplicationManager.USER_ROLE__REVIEWER))
          {
            ...
            addManagerExpertEvent(userAccount);
        }
        catch(Exception ex)
        {
    ...
    public void addManagerExpertEvent(User user)
    {
        Event event = createManagerExpertEvent(user);
        getEventService().insertNewExpertEvent(event);
    }
    
    public Event createManagerExpertEvent(User user)
    {
        Event event = new Event();
        event.setUuid(UUIDUtils.randomUUIDString());
        event.setEventName("Реєстрація нового експерта");
        event.setEventDescription("Зареєстрований експерт: "+ user.getNewSurname() + " " 
                                                            + user.getNewName()    + " ."
                                                            + " Телефон: " + user.getPhone());
        event.setEventTypeId(1);
        
        Date date = DateTimeUtils.setCurrentTimeIntoDate(new Date());
        ...
        Date nexttwoWeekDate = calendar.getTime();
        
        event.setEventEndDate(nexttwoWeekDate);
        return event;
    }
    ...
    public List<UserProgram> getAllProgram()
    {
        return allProgram;
    }

    public void setAllProgram(List<UserProgram> allProgram)
    {

...
return this.pushStack(jQuery(selector).filter(function(){for(i=0;
i<len;
...
for(i=0;
i<len;
i++){jQuery.find(selector,this[i],ret)
...
for(;
(elem=this[i])!=null;
i++){if(!selector||jQuery.filter(selector,[elem]).length>0){if(!keepData&&elem.nodeType===1){jQuery.cleanData(getAll(elem))
